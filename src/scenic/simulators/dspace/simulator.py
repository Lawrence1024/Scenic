...
class DSpaceSimulation(DrivingSimulation):
    def __init__(self, scene, sim: DSpaceSimulator, **kwargs):
        self.sim = sim # Store simulator instance
        self.exp = None
        self.ts  = None
        ...
        self._object_positions = [] # Initialize the list for this simulation instance
        self._csv_filename = None   # Initialize CSV filename tracker for this instance

        # Configuration is correct
        self.config = {
            'lateral_calibration_factor': kwargs.pop('lateral_calibration_factor', 1.0),
            'duplicate_position_threshold': kwargs.pop('duplicate_position_threshold', 0.1),
            'relative_distance_threshold': kwargs.pop('relative_distance_threshold', 50.0),
            # 's_coordinate_threshold': kwargs.pop('s_coordinate_threshold', 5.0), # Unused
            'heading_difference_threshold': kwargs.pop('heading_difference_threshold', 45.0), # Degrees
            'lateral_classification_threshold': kwargs.pop('lateral_classification_threshold', 2.0),
            'longitudinal_calibration_factor': kwargs.pop('longitudinal_calibration_factor', 1.0),
        }
        ...
        print("\nApplying relative positioning fixes...")
        if hasattr(self, '_apply_relative_positioning'):
             self._apply_relative_positioning()
        else:
             print("Warning: DSpaceSimulation missing _apply_relative_positioning method.")

        print("Performing final analysis and logging...")
        if hasattr(self, '_perform_final_analysis_and_log'):
             self._perform_final_analysis_and_log()
        else:
             print("Warning: DSpaceSimulation missing _perform_final_analysis_and_log method.")    
    ...
    def _perform_final_analysis_and_log(self):
        if not hasattr(self, '_object_positions') or len(self._object_positions) < 2:
            print("Not enough objects for final analysis.")
            return

        # Assuming exactly two objects for simplicity
        fellow1_info = self._object_positions[0]
        fellow2_info = self._object_positions[1]

        # Recreate DataFrames with FINAL s/t and stored transformed coords
        f1_df = self.fellow_coords_df(
            fellow1_info['obj'], 1,
            fellow1_info['position'].x, fellow1_info['position'].y,
            fellow1_info['transformed_x'], fellow1_info['transformed_y'],
            fellow1_info['s_coord'], fellow1_info['t_coord']
        )
        f2_df = self.fellow_coords_df(
            fellow2_info['obj'], 2,
            fellow2_info['position'].x, fellow2_info['position'].y,
            fellow2_info['transformed_x'], fellow2_info['transformed_y'],
            fellow2_info['s_coord'], fellow2_info['t_coord']
        )

        # Perform analysis with FINAL s/t values
        combined_df = pd.concat([f1_df, f2_df], axis=0, ignore_index=True)
        ...
        # Create CSV row with FINAL analysis
        row = {
            # Fellow1 (columns 1-11) - Use f1_df
            ...
            'rd_world_y_f1':       f1_df.iloc[0]['rd_world_y'],
            ...
            'rd_world_y_f2':       f2_df.iloc[0]['rd_world_y'],
        ...
        try:
            ...
            print(f"Final analysis written to {self._csv_filename}")
        except Exception as e:
            print(f"Error writing to CSV file {self._csv_filename}: {e}")
    ...
    def fellow_coords_df(self, obj, fellow_idx, scenic_x, scenic_y, transformed_x, transformed_y, s_val, t_val):
        ...
        Columns:
            scene_name: Relative positioning (ex. left of 2)
        ...
        # Scenic's heading convention is meseaured counter-clockwise from +y axis:
        #   +x: right
        #   -x: left
        #   +y: ahead
        #   -y: behind
        # Given heading, vector pointing heading direction = (-sin(heading), cos(heading))
        # Given heading, vector pointing left direction = (-sin(heading + π/2), cos(heading + π/2)) 
    ...
    def create_analysis(self, scenic_x, scenic_y, true_left_x, true_left_y, rd_world_x, rd_world_y, road_s, road_t):
        ...
        # should approximately be the defined relative distance + width of a car (2 by default) without considering the direction
    ...
    def _should_be_relative(self, obj1, obj2):
        ...
        threshold = self.config.get('duplicate_position_threshold', 0.1)
        if world_dist < threshold:
            print(f"  Cars are too close to be relative (identical positions): world_dist={world_dist:.3f}m")
            return False
        
        # --- MODIFIED LOGIC ---
        # Classify the relationship using our (x,y) based function
        # This function must be added to your class (see below)
        rel_type, _ = self._classify_relationship(obj1, obj2)
        is_clear_pattern = rel_type in ("lateral", "longitudinal")
        
        print(f"  Comparing cars: world_dist={world_dist:.1f}m, heading_diff={heading_diff:.2f}rad, pattern_type={rel_type}")
        
        # Criteria for relative positioning:
        # 1. Close in world coordinates
        # 2. Have a clear lateral or longitudinal pattern
        # 3. Similar headings
        
        # Get thresholds from config
        dist_thresh = self.config.get('relative_distance_threshold', 100.0)
        heading_thresh_rad = math.radians(self.config.get('heading_difference_threshold', 45.0))
        
        should_be_relative = (world_dist < dist_thresh and 
                              is_clear_pattern and 
                              heading_diff < heading_thresh_rad)
        
        print(f"  Should be relative: {should_be_relative}")
        return should_be_relative
    
    def _classify_relationship(self, ref_car, target_car):
        """
        Classifies the relationship between two cars as "lateral", "longitudinal", or "diagonal".
        
        Returns:
            (str, float): A tuple of (type, value), where type is one of 
                          ["lateral", "longitudinal", "diagonal"] and value is
                          the projected distance.
        """        
        # Get vector from ref to target
        ref_pos = ref_car['position']
        target_pos = target_car['position']
        dx = target_pos.x - ref_pos.x
        dy = target_pos.y - ref_pos.y

        # Get reference car's heading and vectors
        ref_heading = ref_car['heading']
        
        # Correct Forward Vector (local +Y)
        forward_x = -math.sin(ref_heading)
        forward_y = math.cos(ref_heading)
        
        # Correct Left Vector (local -X)
        left_x = -math.cos(ref_heading)
        left_y = -math.sin(ref_heading)

        # Project the (dx, dy) vector onto the forward and left vectors
        # This gives the longitudinal and lateral components of the offset
        longitudinal_component = dx * forward_x + dy * forward_y
        lateral_component = dx * left_x + dy * left_y

        # Use a ratio to determine if the relationship is "mostly"
        # lateral or "mostly" longitudinal.
        
        # Avoid division by zero
        if abs(longitudinal_component) < 1e-5:
            if abs(lateral_component) < 1e-5:
                return "diagonal", 0  # Cars are at same position
            else:
                return "lateral", lateral_component

        if abs(lateral_component) < 1e-5:
            return "longitudinal", longitudinal_component

        ratio = abs(lateral_component) / abs(longitudinal_component)

        # Define thresholds for classification
        # If lateral is > 2x longitudinal, classify as lateral
        LATERAL_THRESHOLD = 2.0 
        # If longitudinal is > 2x lateral, classify as longitudinal
        LONGITUDINAL_THRESHOLD = 0.5 # (1 / 2.0)

        if ratio > LATERAL_THRESHOLD:
            # Primarily a lateral offset
            return "lateral", lateral_component
        elif ratio < LONGITUDINAL_THRESHOLD:
            # Primarily a longitudinal offset
            return "longitudinal", longitudinal_component
        else:
            # Offset is ambiguous (e.g., 4m left and 3m ahead)
            return "diagonal", 0
    
    def _adjust_group_positions(self, group):
        """
        Adjusts car positions to fix (x,y) -> (s,t) projection errors.
        
        This function classifies the (x,y) offset and forces the
        s- or t-coordinate to match the reference car.
        """
        if len(group) < 2:
            return
            
        print(f"   Adjusting positions for group of {len(group)} cars...")
        
        # Sort the group by s-coordinate (or another stable metric)
        group.sort(key=lambda x: x['s_coord'])
        
        # Use the first car as the reference point
        reference = group[0]
        ...
        # For each subsequent car, classify and adjust
        for i, car in enumerate(group[1:], 1):
            
            # Classify the relationship using (x,y) and heading
            # value is the correct (x,y) offset (e.g., 4.0 or -4.0)
            rel_type, value = self._classify_relationship(reference, car)
            
            if rel_type == "lateral":
                # --- LATERAL FIX ---
                # 'left of' or 'right of' detected.
                
                # calibration_factor = self.config.get('lateral_calibration_factor', 1.0)
                
                # We force 's' to match the reference car.
                # We calculate 't' by adding the CORRECT (x,y) offset ('value')
                # to the reference car's 't'.
                new_s = ref_s  
                new_t = ref_t + value
                # new_t = ref_t + (value * calibration_factor)

                print(f"   Car {i+1}: Lateral pattern ({value:.1f}m). Forcing s={new_s:.3f}, t={new_t:.3f}")
                self._update_fellow_position(car, new_s, new_t)
                
            elif rel_type == "longitudinal":
                # --- LONGITUDINAL FIX ---
                # 'ahead of' or 'behind' detected.
                
                # calibration_factor = self.config.get('longitudinal_calibration_factor', 1.0)

                # We force 't' to match the reference car.
                # We calculate 's' by adding the CORRECT (x,y) offset ('value')
                # to the reference car's 's'.
                new_s = ref_s + value
                # new_s = ref_s + (value * calibration_factor)
                new_t = ref_t  

                print(f"   Car {i+1}: Longitudinal pattern ({value:.1f}m). Forcing t={new_t:.3f}, s={new_s:.3f}")
                self._update_fellow_position(car, new_s, new_t)
                
            else: # rel_type == "diagonal"
                # --- DIAGONAL ---
                # Ambiguous offset. Do not adjust, just use original (s,t).
                print(f"   Car {i+1}: Diagonal pattern. No adjustment applied.")
                self._update_fellow_position(car, car['s_coord'], car['t_coord'])
    ...
    def _update_fellow_position(self, car_info, new_s, new_t=None):
        ...
        car_info['s_coord'] = new_s
        if new_t is not None:
            car_info['t_coord'] = new_t
