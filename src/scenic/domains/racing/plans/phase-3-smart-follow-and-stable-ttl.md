...
- [x] Follow behavior maintains safety gap across benchmarks — validated on dSPACE using `tactical_runner` over `examples/racing/phase3_tactical/` (same seven layouts as Phase 0, ego with `tactical_planner_enabled=True`). Aggregate run: **0 collisions**, **0 off-track**, **0 near-miss events**, all scenarios **`return_code` 0**, laps **completed** at default **3000** steps (~30 s sim). See [Validated benchmarks (dSPACE)](#validated-benchmarks-dspace).
...
```bash
python -m scenic.domains.racing.benchmarks.tactical_runner --inter-run-delay-s 0
```
...
Legacy alias (same scenarios and KPIs; `run_id_prefix=phase3_on_phase0`): `python -m scenic.domains.racing.benchmarks.phase3_on_baseline_runner`.
